...
if(isset($_GET['delpost'])){ 
	...
	try{
     $stmtd = $db->prepare('SELECT postID, postImage, postTitle, postDesc, postCont FROM blog_posts WHERE postID = :postID') ;
...
<body>
  ...
  <div class="container">
  	<div class="row">
  		<div class="col s6 m6 l8 xl8"><h1>Blog Details</h1></div>
			...
			try {
				...
				while($row = $stmt->fetch()){
					...
					echo '<div class ="col s12 m6 l6 xl6 ">';
						echo '<h5><a href="">'.$row['postTitle'].'</a></h3>';
						echo '</div>';
						...
						?>
						<a href="edit-post.php?id=<?php echo $row['postID'];?>"><i class="small material-icons">edit</i></a>  
						<a href="javascript:delpost('<?php echo $row['postID'];?>','<?php echo $row['postTitle'];?>')"><i class="small material-icons">delete</i></a>
						<?php
						echo '</div>';
					echo '</div>';	
